<div class="modal">
    ...
    <form class="form" action="{{ route('rent.update', ['id' => $rent->id ]) }}" method="POST" data-form="valid">
        {{ csrf_field() }}
        
        <div class="form__phrase">
            <label for="phrase">Podaj zwrot</label>
            <textarea rows="5" cols="35" id="phrase" name="phrase">{{ $rent->phrase }}</textarea>
        </div>
        <div class="form__translation">
            <label for="translation">Podaj tłumaczenie</label>
            <textarea rows="5" cols="35" id="translation" name="translation">{{ $rent->translation }}</textarea>
        </div>
